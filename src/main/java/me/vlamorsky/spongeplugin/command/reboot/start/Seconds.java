...
public class Seconds implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if (reasonOP.isPresent()) {
            TimeCheckerThread.setRestartDateTime(LocalDateTime.now().plusSeconds(timeAmount), reasonOP.get());
        } else {
            TimeCheckerThread.setRestartDateTime(LocalDateTime.now().plusSeconds(timeAmount));
        }
